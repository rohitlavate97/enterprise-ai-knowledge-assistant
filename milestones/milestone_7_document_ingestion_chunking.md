...
## Status
- [ ] Install and configure `pypdf` dependency (`requirements.txt`)
- [ ] Create text extraction adapter for TXT, Markdown, and PDF formats (`app/services/document_parser.py`)
- [ ] Create recursive sliding-window semantic text chunker (`app/services/document_chunker.py`)
- [ ] Update background document processing task to orchestrate parsing and chunking (`app/services/document_processing.py`)
- [ ] Write integration and pipeline tests validating parsing formats, chunk sizes, and overlaps
- [ ] Verify MyPy typing and Ruff linting rules pass cleanly
...
* *Commit 19 (Planned):* Implement document parser, semantic chunker, and coordinate processing worker.
* *Commit 20 (Planned):* Write test suites for text extraction and chunking pipeline.
